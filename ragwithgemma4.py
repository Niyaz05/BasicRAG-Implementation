...
A basic RAG (Retrieval-Augmented Generation) pipeline that:
  ...
  6. Sends query + context to a LOCAL Gemma 4 model (via Ollama) for answer generation

SETUP (run once in your terminal):
    pip install requests beautifulsoup4 sentence-transformers faiss-cpu ollama

Make sure Ollama is running and you've pulled a Gemma 4 tag, e.g.:
    ollama pull gemma4          # or gemma4:12b / gemma4:26b depending on your RAM
...
# ─────────────────────────────────────────────
OLLAMA_MODEL   = "gemma4"   # change to the exact tag you pulled, e.g. "gemma4:12b"
OLLAMA_NUM_CTX = 8192       # Ollama defaults Gemma 4 to 4K ctx — override so context isn't truncated
...
# ─────────────────────────────────────────────────────────────────
# STEP 6 — GENERATE ANSWER WITH LOCAL GEMMA 4 (via Ollama)
# Build a prompt that contains the retrieved chunks as "context",
# then ask Gemma to answer ONLY from that context.
# This is the "Augmented Generation" part of RAG — now fully local.
# ─────────────────────────────────────────────────────────────────
def step6_generate_answer(query: str, retrieved_chunks: list[str], model_name: str, num_ctx: int) -> str:
    print("\n" + "="*60)
    print("STEP 6: GENERATING ANSWER WITH LOCAL GEMMA 4 (Ollama)")
    print("="*60)
    ...
    system_prompt = "Answer the user's question using ONLY the provided context. If the context doesn't contain the answer, say so."
    user_prompt = f"Context:\n{context}\n\nQuestion:\n{query}"

    print(f"  → Prompt length: {len(user_prompt):,} characters")
    print("  → Sending to local Gemma 4 via Ollama...")
    ...
    response = ollama.chat(
        ...
        options={"num_ctx": num_ctx},  # override Ollama's 4K default for Gemma 4
    )
...
def main():
    print("\n" + "█"*60)
    print("  RAG PIPELINE — Python Docs + Local Gemma 4 (Ollama)")
    print("█"*60)
    ...
    # ── Step 6: Generate ──────────────────────────────────────────
    answer = step6_generate_answer(QUERY, retrieved_chunks, OLLAMA_MODEL, OLLAMA_NUM_CTX)
